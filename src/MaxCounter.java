public class MaxCounter {
	...
	public static void main(String[] args) {
		...
		int[] sol = solution(5, new int[] { 3, 4, 4, 6, 1, 4, 4 });

		;
		for (int i = 0; i < sol.length; System.out.print(sol[i++] + ", ")) {
	...
	public static int[] solution(int N, int[] A) {
		...
		int current, maxValue = 0, resetToMax = 0;
		for (int i = 0; i < A.length; i++) {
			...
			if (current == N) {
				...
				if (result[current] <= resetToMax)
					result[current] = resetToMax + 1;
				else result[current]++;
					
				if (maxValue < result[current])
					maxValue = result[current];
			}
		}
		
		for (int i = 0; i < result.length; i++) {
			if(result[i] < resetToMax) result[i] = resetToMax;
		}
