...
public class OddOccurrencesInArray {
	...
	public static void main(String[] args) {
		...
		int sol = solution(new int[] { 9,3,9,3,9,7,9});
	...
	public static int solution(int[] A) {
		...
		for (int i = 0; i < A.length; i++) {
			if (mapNumbers.containsKey(A[i])) {
				mapNumbers.remove(A[i]);
			} else {
				mapNumbers.put(A[i], true);
			}
		}
		
		return mapNumbers.keySet().iterator().next();
 ...
 * 
 * For example, given array A such that: A[0] = 9 A[1] = 3 A[2] = 9 A[3] = 3
 * A[4] = 9 A[5] = 7 A[6] = 9
 * 
